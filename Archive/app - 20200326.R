...
dfUS <- data %>% dplyr::filter(
  ...
  cases >= 100 # start monitoring after 10 cases
...
ylvl <- c(5000, 1000, 500)
funcLoc <- tibble(
  days = do.call('c', Map(function(y, k) {(log10(y) - 2) * k/log10(2)}, y = as.list(ylvl), k = as.list(c(2, 3, 5)))),
  cases = ylvl,
  ...
  output$lineChartUS <- renderPlot({
    ...
    ggplot(usedData, aes(x = days, y = cases, group = `Province/State`)) +
      ...
      geom_text(
        ...
        nudge_x = 0.5,
        nudge_y = log10(1)
      ) + scale_y_log10(labels = scales::comma, limits = c(100, max(dfUS$cases) * 2)) +
      scale_x_continuous(limits = c(0, max(dfUS$days) + 3), breaks = seq(0, max(dfUS$days) + 3, by = 2)) +
      stat_function(fun = exp_growth, args = list(k = 5, x0 = 100), colour = "#ee9e64", alpha = 0.5) +
      stat_function(fun = exp_growth, args = list(k = 3, x0 = 100), colour = "#ee9e64", alpha = 0.5) +
      stat_function(fun = exp_growth, args = list(k = 2, x0 = 100), colour = "#ee9e64", alpha = 0.5) +
      xlab("Days since 100th confirmed cases") + ylab("Total Confirmed Cases")
  }, width = 800, height = 600)
}
